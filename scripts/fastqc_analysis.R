...
get_fastqc_data <- function(filepath, grouping = FALSE) {
...
# Function that creates and saves plots from fastqc data
create_plots_nogroups <- function(df_list) {
  p1 <- df_list$adapter_content %>%
    # get data ready for boxplot by gathering the columns into two columns
    gather(key,
           value,-c(ref,
  ...
  p3 <- df_list$sequence_duplication_levels %>%
    gather(key, value,-c(ref, `Duplication Level`)) %>%
    ggplot(aes(
      factor(
        `Duplication Level`,
        levels = unique(`Duplication Level`),
...
if (group_var == TRUE) {
  ...
  # Data and plotting
  df_list <- get_fastqc_data(report_loc, grouping = TRUE)
